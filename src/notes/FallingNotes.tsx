...
export function FallingNotes() {
  ...
  useFrame(() => {
    ...
    // Suppress upward-flying live note trails while a song is actively playing
    // to prevent them from visually clashing with the downward-falling bars.
    // They will only appear when playing solo (paused or stopped).
    if (liveNotes.length > 0 && useStore.getState().transport !== 'playing') {
      const liveNow = now()
