...
export async function parseMidi(file: ArrayBuffer, name: string): Promise<ParsedSong> {
  ...
  const tempos = midi.header.tempos.map(t => ({ time: t.time ?? midi.header.ticksToSeconds(t.ticks), bpm: t.bpm }))
  const timeSignatures = midi.header.timeSignatures.map(ts => ({ time: midi.header.ticksToSeconds(ts.ticks), timeSignature: ts.timeSignature }))
  let id = 0
