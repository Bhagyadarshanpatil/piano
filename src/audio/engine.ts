...
export class AudioEngine {
  ...
  // When true, song-scheduled notes are silenced (tick() uses noopStop
  // instead of piano.start()) but key-on EVENTS still fire for visuals.
  // triggerKey() (user key presses) is NOT affected — those always
  // play at full volume. This is the practice-mode gate.
  private songPlaybackMuted = false
  private loop = false
  ...
  /**
   * Mute / unmute only the auto-scheduled (song) notes.
   * When muted: tick() emits key-on events (so glow / particles / bars
   * still fire) but skips piano.start() — so falling bars are visible
   * but play silently. triggerKey() (user presses) is NEVER affected
   * and always plays at full sampler volume.
   * Use this for practice mode so the user hears only what they play.
   */
  setSongPlaybackMuted(muted: boolean): void {
    this.songPlaybackMuted = muted
  }
  ...
  tick(): void {
    ...
    while (this.noteIdx < this.song.notes.length && this.song.notes[this.noteIdx].time <= midiSongTime) {
      ...
      const shaped = this.shapeVelocity(n.velocity)
      const stopFn =
        this.silent || !this.piano || this.songPlaybackMuted
          ? noopStop
      ...
      this.active.set(n.id, { id: n.id, midi: playedMidi, endTime, stop: stopFn })
      // In practice mode, suppress the key-on event so the keyboard glow /
      // landing flash / hit particles don't fire automatically. Falling bars
      // are still visible (they read song data from the store, not events).
      // Only user-pressed keys (triggerKey) will produce glow in this mode.
      if (!this.songPlaybackMuted) {
